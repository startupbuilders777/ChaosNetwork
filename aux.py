...
def tie_breaking_algo_test_final():
    ...
    print(input_selected_field_nodes_arr.stack().eval())
    print(input_selected_field_nodes_arr.read(0).eval())
    print(input_selected_field_nodes_arr.read(1).eval())
    print(input_selected_field_nodes_arr.read(2).eval())
    ...
    def find_weight_matched_nodes_body(index, output_arr):
        printIndex = tf.Print(index, [index], "index: ")
        input_node_matching = input_selected_field_nodes_arr.read(printIndex)
        node_id = tf.gather(input_node_matching, 0)
        
        weight_match = tf.cast(tf.gather(input_node_matching, 1), tf.int32) 

    
        def find_weight_match_cond(weight_match, keep_going):
            return keep_going
             

        def find_weight_match_body(weight_match, keep_going):
            #keep_going is our variable, we just return it in the cond
            taken = -1 #taken_arr.read(weight_match)
            # taken_print = tf.Print(taken, [taken], "yee: ")
            # weight_match_print = tf.Print(weight_match, [weight_match], "weight match val: ")
        ...
        output_arr_changed = output_arr.write(index, node_id)
                            
        return (index + 1, output_arr_changed)

    index_final, final_weight_matched_nodes_arr = tf.while_loop(find_weight_matched_nodes_cond, 
                                                                find_weight_matched_nodes_body, 
                                                                loop_vars=(index, weight_matched_nodes_arr), 
                                                                                shape_invariants=None,
                                                                                parallel_iterations=1, 
